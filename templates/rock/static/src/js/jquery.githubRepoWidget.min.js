	...
	d(".github-widget").each(function() {
		...
		d.ajax({
			...
			success: function(n) {
				...
				k.find(".watchers").text(m.watchers);
				k.find(".forks").text(m.forks);
				k.find(".description span").text(m.description);
				k.find(".updated").html("Latest commit to the <strong>master</strong> branch on " + o);
				if (m.homepage != null) {
					k.find(".link").append(d("<a />").attr("href", m.homepage).text(m.homepage))
				}
			}
		})
	})
